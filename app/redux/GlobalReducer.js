...
export default createModule({
    ...
    transformations: [
        ...
        },
        {
            ...
            reducer: (state, { payload: { nft_collections, nft_assets, assets } }) => {
                let new_state = state
                new_state = new_state.set('nft_market_collections', fromJS(nft_collections))
                if (nft_assets) {
                    new_state = upsertNftAssets(new_state, nft_assets)
                }
                new_state = new_state.set('assets', fromJS(assets))
                return new_state
            },
        },
        {
        ...
        },
        {
            ...
            reducer: (state, { payload: { nft_orders, start_order_id, next_from, nft_assets } }) => {
                let new_state = state
        ...
        {
            action: 'FETCH_OWN_NFT_ORDERS',
            reducer: state => state,
        },
        {
            action: 'RECEIVE_OWN_NFT_ORDERS',
            reducer: (state, { payload: { own_nft_orders, start_order_id, next_from, nft_assets } }) => {
                let new_state = state
                new_state = upsertOwnNftOrders(new_state, own_nft_orders, start_order_id, next_from)
                if (nft_assets) {
                    new_state = upsertNftAssets(new_state, nft_assets, start_order_id)
                }
                return new_state
