...
export function* fetchDataWatches () {
    ...
    yield fork(watchFetchOwnNftOrders)
}
        ...
        if (parts[0][0] === '@') {
            ...
            if (account) {
                switch (parts[1]) {
                    ...
                    break
                    case 'nft-history':
                        const nftHistory = yield call([api, api.getAccountHistoryAsync], uname, -1, 1000, {select_ops: ['nft_token', 'nft_transfer', 'nft_sell', 'nft_buy']})
                        account.nft_history = []
            ...
            if (parts[1]) {
                ...
                try {
                    ...
                    if (state.nft_token) {
                        ...
                        const price = Asset(state.nft_token.last_buy_price)
                        syms.add(price.symbol)
                    }
...
export function* fetchNftMarketCollections({ payload: { } }) {
    try {
        const limit = 99

        const nft_collections = yield call([api, api.getNftCollectionsAsync], {
            limit: limit + 1,
            sort: 'by_created'
        })

        let next_from
        if (nft_collections.length > limit) {
            next_from = nft_collections.pop().order_id
        }

        const nft_assets = {}

        try {
            yield fillNftCollectionImages(nft_collections)

            const syms = new Set()

            for (const nco of nft_collections) {
                nco.image = nco.image || NFTImageStub()
        ...
        let assets = yield call([api, api.getAssetsAsync], '', [], '', 5000, 'by_marketed')

        yield put(GlobalReducer.actions.receiveNftMarketCollections({nft_collections, nft_assets, assets}))
    } catch (err) {
...
export function* fetchNftOrders({ payload: { account, collectionName, start_order_id, sort, reverse_sort } }) {
    try {
        const limit = 20
        ...
        yield put(GlobalReducer.actions.receiveNftOrders({nft_orders, start_order_id, next_from, nft_assets}))
    } catch (err) {
        console.error('fetchNftOrders', err)
    }
}

export function* watchFetchOwnNftOrders() {
    yield takeLatest('global/FETCH_OWN_NFT_ORDERS', fetchOwnNftOrders)
}

export function* fetchOwnNftOrders({ payload: { account, collectionName, start_order_id, sort, reverse_sort } }) {
    try {
        const limit = 20

        const nft_own_orders = yield call([api, api.getNftOrdersAsync], {
            owner: account,
            select_collections: collectionName ? [collectionName] : undefined,
            start_order_id,
            type: 'selling',
        ...
        let next_from
        if (nft_own_orders.length > limit) {
            next_from = nft_own_orders.pop().order_id
        }
        ...
        yield put(GlobalReducer.actions.receiveOwnNftOrders({nft_own_orders, start_order_id, next_from, nft_assets}))
    } catch (err) {
        console.error('fetchOwnNftOrders', err)
    }
}
