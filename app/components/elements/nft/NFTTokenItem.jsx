...
class NFTTokenItem extends Component {
    ...
    render() {
        ...
        let last_price
        const price = token.selling ? Asset(token.order.price) : Asset(token.last_buy_price)
        if (price.amount > 0) {
            ...
            last_price = <span title={(selling ? tt('nft_tokens_jsx.selling_for') : '') + price.floatString}>
        ...
        if (last_price) {
            buttons = <div>
                ...
                {isMy && !selling && <button className='button slim float-right'>{tt('g.sell')}</button>}
            ...
            buttons = <div>
                {page === 'collection' && !isMy && <Link to={'/@' + token.owner}>
                    {'@' + token.owner}
                </Link>}
                &nbsp;
